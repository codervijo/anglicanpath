...
// Ported from genai/src/components/office/office-reader.tsx. Mounted as an
// Astro island; TanStack <Link> became plain <a href> with trailing slashes.
const officeHref = (book: string, office: string, date: string) => `/office/${book}/${office}/${date}/`;
    ...
    <div className="office-toolbar sticky top-18 z-30 border-b border-border bg-background/95">
      <div className="mx-auto grid max-w-6xl gap-3 px-5 py-3 sm:px-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div className="grid min-w-0 grid-cols-2 gap-2 sm:flex sm:flex-wrap">
          <Button variant="outline" className="h-11" asChild><a href={officeHref(book === "1928" ? "1662" : "1928", office, date)}>{book} BCP</a></Button>
          <Button variant="outline" className="h-11" asChild><a href={officeHref(book, office === "morning" ? "evening" : "morning", date)}>{title}</a></Button>
